...
class FastDepthV2(nn.Module):
  ...
  def forward(self,x):
    ...
    x = F.interpolate(self.decoder.conv1(x), size=layer3.shape[2:], mode='nearest')
    x = F.interpolate(self.decoder.conv2(x), size=layer3.shape[2:], mode='nearest')
    x = x + layer3

    # x= F.interpolate(self.decoder.conv3(x), scale_factor=2, mode='nearest')
    # x = x+layer2

    x = F.interpolate(self.decoder.conv3(x), size=layer2.shape[2:], mode='nearest')
    x = x + layer2

    # x= F.interpolate(self.decoder.conv4(x), scale_factor=2, mode='nearest')
    # x = x+layer1

    x = F.interpolate(self.decoder.conv4(x), size=layer1.shape[2:], mode='nearest')
    x = x + layer1
